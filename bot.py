...
async def generate_image_prompt(image_base64: str, prompt_type: str, language: str) -> str:
    system = SYSTEM_PROMPTS.get("image_analysis", SYSTEM_PROMPTS["image_analysis"])
    lang_full = LANG_NAMES_FULL.get(language, "English")
    system = system.replace("{output_language}", lang_full)
    ...
    try:
        response = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": system},
                {"role": "user", "content": f"Analyze this image and generate a professional {prompt_type} prompt in {lang_full}."}
            ],
            temperature=0.85,
            max_tokens=1200,
        )
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    udata = get_user(user_id)
    lang = udata.get("language", "en")
    step = udata.get("step", "new")

    # بررسی عضویت
    is_member = await check_membership(user_id, context)
    if not is_member:
        await update.message.reply_text(
            "❌ عمویی، اول عضو کانال بشو!\nDear, join the channel first!",
            reply_markup=get_join_keyboard()
        )
        return
    ...
    # پردازش متن
    if step == "waiting_text" and update.message.text:
        prompt_type = udata.get("prompt_type", "image")
        processing_msg = await update.message.reply_text(get_msg(lang, "processing"))
        
        result = await generate_text_prompt(update.message.text, prompt_type, lang)
        
        await processing_msg.delete()
        
        if result:
            await update.message.reply_text(
                f"{get_msg(lang, 'result_header')}\n\n{result}\n\n{get_msg(lang, 'result_footer')}",
                parse_mode="Markdown",
                reply_markup=get_result_keyboard(lang)
            )
        else:
            await update.message.reply_text(get_msg(lang, "error"))
        return

    # پردازش عکس
    if step == "waiting_image" and update.message.photo:
        prompt_type = udata.get("prompt_type", "image")
        processing_msg = await update.message.reply_text(get_msg(lang, "processing"))
        ...
        result = await generate_image_prompt(image_data, prompt_type, lang)
    ...
    # اگه هیچ کدوم از حالت‌های بالا نبود
    if step == "main_menu" or step == "new":
        msg = get_msg(lang, "choose_type")
